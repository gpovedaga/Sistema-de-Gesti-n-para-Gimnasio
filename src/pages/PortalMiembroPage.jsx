...
export default function PortalMiembroPage() {
    ...
    const [tab, setTab] = useState('clases') // 'clases' | 'reservas' | 'membresia'
    ...
    const fetchAll = async () => {
        try {
            const [c, r] = await Promise.allSettled([
                ...
                reservaService.porMiembro(doc),  // ← solo las del miembro
            ])
    ...
    const fetchMembresia = async () => {
        try {
            const res = await membresiaService.porMiembro(doc)  // ← solo las del miembro
            const lista = Array.isArray(res.data) ? res.data : []
            setMembresia(lista.find(m => m.estado === 'activa') || null)
        } catch { }
    }

    useEffect(() => { if (tab === 'membresia') fetchMembresia() }, [tab])
    ...
    /* Color del chip de estado de reserva */
    ...
    return (
        <div className="portal-wrapper">
            ...
            <div className="portal-content">
                ...
                {/* Mensaje de feedback */}
                {msg && (
                    <div className={`portal-msg ${msg.startsWith('✅') ? 'success' : 'error'}`}>
                        {msg}
                    </div>
                )}
