...
const EMPTY = { documento:'', nombreCompleto:'', correo:'', password:'', telefono:'', cargo:'recepcionista', estado:'activo' }
...
export default function EmpleadosPage() {
  ...
  useEffect(() => { fetch() }, [])
  const fetch = async () => { setLoading(true); try { const r = await empleadoService.listar(); setData(r.data) } finally { setLoading(false) } }
  ...
  const save = async e => {
    ...
    } catch (err) { setError(err.response?.data || 'Error al guardar.') } finally { setSaving(false) }
  }

  const del = async () => { try { await empleadoService.eliminar(confirm); fetch() } finally { setConfirm(null) } }
  ...
  return (
    <div className="animate-fade">
      <PageHeader title="EMPLEADOS" subtitle={`${data.length} empleados registrados`} action={<Button icon={Plus} onClick={openCreate}>Nuevo empleado</Button>} />
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'EDITAR EMPLEADO' : 'NUEVO EMPLEADO'}>
        ...
        <form onSubmit={save}>
          <div className="form-grid">
            ...
            <FormInput label="Correo *" type="email" value={form.correo} onChange={e=>setForm(f=>({...f,correo:e.target.value}))} required />
            <FormInput label={editing ? 'Nueva contraseña (opcional)' : 'Contraseña *'} type="password" value={form.password} onChange={e=>setForm(f=>({...f,password:e.target.value}))} required={!editing} />
            <FormInput label="Teléfono" value={form.telefono} onChange={e=>setForm(f=>({...f,telefono:e.target.value}))} />
            <FormSelect label="Cargo" value={form.cargo} onChange={e=>setForm(f=>({...f,cargo:e.target.value}))} options={CARGO_OPTS} />
