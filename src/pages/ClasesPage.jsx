          ...
          <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'EDITAR CLASE' : 'NUEVA CLASE'} width={640}>
            ...
            <form onSubmit={save}>
              <div className="form-grid">
                ...
                <FormInput label="Hora inicio *" type="time" value={form.horaInicio} onChange={e => setForm(f => ({ ...f, horaInicio: e.target.value.substring(0, 5) }))} required />                <FormSelect label="Instructor *" value={form.instructorIdEmpleado} onChange={e => setForm(f => ({ ...f, instructorIdEmpleado: e.target.value }))} options={instructores} required />
